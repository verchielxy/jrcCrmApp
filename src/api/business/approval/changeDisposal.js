...
const apiVersion = '/api/v1/disposalCheck';

const approvalChangeDisposalApi = {
    ...
    return request({
      ...
      formatFunction: (res) => {
        ...
        json.records.map(function (item, index) {
          ...
          item.typeText = constant.convertText(constant.BUSINESS.CHANGE_DISPOSAL.TYPE, item.type);
  ...
  view(id, searchParams) {
    return request({
      ...
      formatFunction: (res) => {
        ...
        json.typeText = constant.convertText(constant.BUSINESS.CHANGE_DISPOSAL.TYPE, json.type);
...
export default approvalChangeDisposalApi;
