    ...
    <view class="relative" v-if="target" style="padding-bottom: 75px;">
      <view class="bg-white mb20px">
        ...
        <up-cell-group>
          ...
          <up-cell title="工单备注">
            <template #value>
              <text class="right-text" style="max-width: 70%;">{{ target.remarks }}</text>
            </template>
          </up-cell>
      ...
      <view class="bg-white mb20px" v-if="targetMaintainItems && targetMaintainItems.length > 0">
        <view class="p15px" style="border-bottom: 1px solid #eee;">
          <MaterialSymbol class="middle-icon c-primary" name="model_training" :size="20" type="sharp"></MaterialSymbol>
          保养的项目
        </view>

        <view style="border-bottom: 1px solid #eee;">
          <up-collapse>
            <up-collapse-item :title="item.name" name="Docs guide" v-for="item in targetMaintainItems">
              <view style="margin: -12px 0;">
                <up-cell-group :border="false" :customStyle="{'padding': '0'}">
                  <up-cell title="编号">
                    <template #value>
                      <text class="right-text">{{ item.code }}</text>
                    </template>
                  </up-cell>
                  ...
                  <up-cell title="等级">
                    <template #value>
                      <text class="right-text">{{ item.gradeText }}</text>
                    </template>
                  </up-cell>

                  <up-cell title="执行方式">
                    <template #value>
                      <text class="right-text">{{ item.methodText }}</text>
                    </template>
                  </up-cell>

                  <up-cell title="保养部位">
                    <template #value>
                      <text class="right-text" style="max-width: 70%;">{{ item.part }}</text>
                    </template>
                  </up-cell>

                  <up-cell title="保养周期">
                    <template #value>
                      <text class="right-text">{{ item.cycleVal }} {{ item.cycleUnitText }}</text>
                    </template>
                  </up-cell>
                </up-cell-group>
              </view>
...
export default defineComponent({
  ...
  setup() {
    ...
    const api = proxy.$api.approvalMaintainWorkOrder;
    const loading = ref(false);
    const target = ref();
    const targetEquipment = ref([]);
    const targetMaintainItems = ref([]);
    ...
    const getMaintainItems = () => {
      let existIds = target.value.orderItemList?.map(item => item.itemId);

      if (existIds && existIds.length > 0) {
        proxy.$api.common.maintainItems(1, -1,{
          existIds: existIds,
        }).then((res) => {
          const json = res.result;

          targetMaintainItems.value = json.records;
