      ...
      <up-list-item class="mb10px" v-for="(item, index) in data" :key="index">
        <view class="data-item bg-white radius3px">
          ...
          <up-cell-group>
            <up-cell title="处置方式">
              <template #value>
                <text class="right-text">{{ item.typeText }}</text>
              </template>
            </up-cell>
            ...
            <up-cell title="申请时间">
              <template #value>
                <text class="right-text">{{ item.applyDate }}</text>
              </template>
            </up-cell>

            <up-cell title="申请理由">
              <template #value>
                <text class="right-text" style="max-width: 80%;">{{ item.applyReason }}</text>
              </template>
            </up-cell>
...
export default defineComponent({
  ...
  setup() {
    ...
    const api = proxy.$api.approvalChangeDisposal;
    ...
    const handleClick = (item) => {
      jumpTo({
        url: '/pages/approval/changeDisposal/view',
        params: {
          id: item.id,
        }
      })
