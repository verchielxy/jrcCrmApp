    ...
    <view class="relative" v-if="target" style="padding-bottom: 75px;">
      <view class="bg-white mb20px">
        ...
        <up-cell-group>
          ...
          <up-cell title="工单生成方式">
            <template #value>
              <text class="right-text">{{ target.maintenance }}</text>
            </template>
          </up-cell>

          <up-cell title="提前生成时间">
            <template #value>
              <text class="right-text">{{ target.createAdv }} 日</text>
            </template>
          </up-cell>

          <up-cell title="工单有效时长">
            <template #value>
              <text class="right-text">{{ target.orderVid }} 日</text>
            </template>
          </up-cell>
            ...
            <up-collapse-item :title="item.name" name="Docs guide" v-for="item in targetEquipments">
              <view style="margin: -12px 0;">
                <up-cell-group :border="false" :customStyle="{'padding': '0'}">
                  ...
                  <up-cell title="运行状态">
                    <template #value>
                      <text class="right-text">{{ item.runningStatusText }}</text>
                    </template>
                  </up-cell>

                  <up-cell title="使用状态">
                    <template #value>
                      <text class="right-text">{{ item.useStatusText }}</text>
                    </template>
                  </up-cell>
                </up-cell-group>
              </view>
...
export default defineComponent({
  ...
  setup() {
    ...
    const api = proxy.$api.approvalMaintainPlan;
    ...
    const getMaintainItems = () => {
      let existIds = target.value.itemList?.map(item => item.itemId);
