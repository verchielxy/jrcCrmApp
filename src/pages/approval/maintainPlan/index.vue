        ...
        <view class="data-item bg-white radius3px">
          ...
          <up-cell-group>
            ...
            <up-cell title="工单生成方式">
              <template #value>
                <text class="right-text">{{ item.maintenance }}</text>
              </template>
            </up-cell>

            <up-cell title="提前生成时间">
              <template #value>
                <text class="right-text">{{ item.createAdv }} 日</text>
              </template>
            </up-cell>

            <up-cell title="工单有效时长">
              <template #value>
                <text class="right-text">{{ item.orderVid }} 日</text>
              </template>
            </up-cell>
...
export default defineComponent({
  ...
  setup() {
    ...
    const handleClick = (item) => {
      jumpTo({
        url: '/pages/approval/maintainPlan/view',
        params: {
          id: item.id,
        }
      })
