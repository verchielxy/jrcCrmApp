    ...
    >
      <up-list-item class="mb10px" v-for="(item, index) in data" :key="index">
        <view class="data-item bg-white radius3px">
          <view :class="['data-item-header c-white p15px', item.rankConstant ? 'bg-' + item.rankConstant.color : '']">
            {{ item.rankText }} <text class="mx7px">|</text> {{ item.code }}
          </view>

          <up-cell-group>
            <up-cell title="设备名称">
              <template #value>
                <text class="right-text">{{ item.device?.name }}</text>
              </template>
            </up-cell>
            ...
            <up-cell title="运行状态">
              <template #value>
                <u-tag v-if="item.runningStatus === 1" plain>{{ item.runningStatusText }}</u-tag>
                <u-tag v-if="item.runningStatus === 2" plain type="warning">{{ item.runningStatusText }}</u-tag>
                <u-tag v-if="item.runningStatus === 3" plain type="error">{{ item.runningStatusText }}</u-tag>
              </template>
            </up-cell>

            <up-cell title="工单时间">
              <template #value>
                <text class="right-text">{{ item.startDate }} ~ {{ item.endDate }}</text>
              </template>
            </up-cell>
            ...
            <up-cell>
              ...
              <template #value>
                <view style="margin: -5px 0;">
                  <up-button style="width: auto;" type="primary" @click="handleClick(item)">查看详情</up-button>
                </view>
              </template>
            </up-cell>
          </up-cell-group>
...
export default defineComponent({
  ...
  setup() {
    ...
    const handleClick = (item) => {
      uni.setStorageSync('temp_data_item', item);

      jumpTo({
        url: '/pages/my/repair/view',
        params: {
          id: item.id,
        }
      })
