<template>
  <uPage>
    ...
    >
      ...
      <up-loadmore :status="pagination2.status" @loadmore="loadmore" />
    </up-list>
  </uPage>
</template>
...
export default defineComponent({
  ...
  setup() {
    ...
    const api = proxy.$api.reviewEngineeringContract;
    const api2 = proxy.$api.reviewEngineeringContract.result;
    ...
    const handleClick = (item) => {
      jumpTo({
        url: '/pages/business/review/contract/engineering/view',
        params: {
          id: item.id,
          tab: tabActive.value,
        }
