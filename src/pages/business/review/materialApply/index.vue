    ...
    >
      <up-list-item class="mb10px" v-for="(item, index) in data" :key="index">
        <view class="data-item bg-white p10px radius3px">
          ...
          <view class="data-item-line">
            <text>收货人：</text>{{ item.recipient }} {{ item.recipientPhone }}
          </view>
          <view class="data-item-line">
            <text>收货地址：</text>{{ item.address }}
          </view>
        </view>
      </up-list-item>
...
export default defineComponent({
  ...
  setup() {
    ...
    const api2 = proxy.$api.reviewMaterialApply.result;
    ...
    const handleClick = (item) => {
      jumpTo({
        url: '/pages/business/review/materialApply/view',
        params: {
          id: item.id,
          tab: tabActive.value,
        }
