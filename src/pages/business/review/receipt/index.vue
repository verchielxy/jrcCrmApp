      ...
      <up-list-item class="mb10px" v-for="(item, index) in data" :key="index">
        <view class="data-item bg-white p10px radius3px">
          ...
          <view class="data-item-line">
            <text>供应商：</text>{{ item.supplierName }}
          </view>
          <view class="data-item-line">
            <text>付款时间：</text>{{ item.payTime }}
          </view>
          <view class="data-item-line">
            <text>付款金额：</text>{{ item.payMoney }}
          </view>
        </view>
      </up-list-item>
...
export default defineComponent({
  ...
  setup() {
    ...
    const api2 = proxy.$api.reviewReceipt.result;
    ...
    const handleClick = (item) => {
      jumpTo({
        url: '/pages/business/review/receipt/view',
        params: {
          id: item.id,
          tab: tabActive.value,
        }
